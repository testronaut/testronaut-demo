    ...
    const _mountRecipeSearch = async () => {
      ...
      return {
        getMealPlannerRecipes: () =>
          runInBrowser('get MealPlanner recipes', () => TestBed.inject(MealPlanner).recipes()),
        addRecipeToMealPlanner: (recipe: Recipe) =>
          runInBrowser('add Recipe to MealPlanner', { recipe }, ({ recipe }) => {
...
test.describe('RecipeSearch', () => {
  test('shows recipes', async ({ page, mountRecipeSearch }) => {
    ...
    await expect(page.getByRole('heading', { level: 2 })).toHaveText(['Burger', 'Salad']);
  });
